...
import { useEffect, useRef, useState } from "react";
import { View, TouchableOpacity, Dimensions, Animated, StatusBar, Platform, ScrollView } from "react-native";
import { useRouter } from "expo-router";
import { Text } from '@/components/AppText'
import AsyncStorage from "@react-native-async-storage/async-storage";
import tw from "twrnc";
...
export default function Index() {
    ...
    // recalculate dimensions after mount so web gets correct values
    useEffect(() => {
    ...
    return (
        <View style={[tw`flex-1 bg-[#0B0B0B]`, { minHeight: height }]}>
            {/* Skip */}
            <View style={[tw`items-end justify-end px-8 pb-2`, { height: SKIP_H }]}>
                <TouchableOpacity onPress={() => completeOnboarding(router)} style={tw`px-4 py-2`}>
                    <Text style={tw`text-gray-400 text-base`}>Skip</Text>
            ...
            {/* Single slide view — works on both web and mobile */}
            <View style={{
                width,
                height: SLIDE_H,
                alignItems: 'center',
                ...
                {slide.type === 'text' ? (
                    <View style={{ alignItems: 'center', marginBottom: gapSm }}>
                        ...
                        <Text style={{ fontSize: emojiSize }}>{slide.emoji}</Text>
                    </View>
                ) : (
                    <View style={{ marginBottom: gapSm }}>
                        <View style={tw`items-center`}>
                            ...
                            {slide.frameLabel && (
                                <Text style={tw`text-xs text-gray-500 mt-2`}>{slide.frameLabel}</Text>
                            )}
                        </View>
                    </View>
                )}
